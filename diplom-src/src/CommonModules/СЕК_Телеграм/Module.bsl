Процедура ОтправкаУведомленийТелеграм()  Экспорт
	...
	Выборка = Справочники.СЕК_УведомленияТелеграмБоту.Выбрать();
...
Процедура ОтправитьСообщениеВТелеграм(ТекстСообщения, ЕстьСвязь)
	...
	ИдентификаторГруппы = Константы.СЕК_ИдентификаторГруппыДляОповещения.Получить(); 
	
	// создайте HTTP-запрос к ресурсу /bot[ВашТокен]/sendMessage
	Токен = Константы.СЕК_ТокенУправленияТелеграмБотом.Получить(); 
